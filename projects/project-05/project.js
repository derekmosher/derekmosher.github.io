...
async function loadPortfolioConfig() {
  try {
    // Resolve current project folder robustly
    const parts = window.location.pathname.split('/').filter(Boolean);
    // Find a folder that matches project-NN pattern, fallback to second-last
    let projectFolder = null;
    for (let i = parts.length - 2; i >= 0; i--) {
      if (/^project-\d+/i.test(parts[i])) { projectFolder = parts[i]; break; }
    }
    if (!projectFolder) projectFolder = parts[parts.length - 2] || parts[parts.length - 1];

    // Try several likely locations for config.json
    const candidates = ['../config.json', '/projects/config.json', '/config.json'];
    let config = null;
    ...
    if (response.ok) {
      ...
      if (Array.isArray(manifestItems) && manifestItems.length > 0) {
        ...
        for (const item of manifestItems) {
          try {
            const imgResponse = await fetch(item.thumbnail, { method: 'HEAD' });
            if (imgResponse.ok) {
              verifiedItems.push(item);
            }
          } catch (e) {
            // Image doesn't exist, skip it
            console.log(`Image not found: ${item.thumbnail}`);
          }
        }
        ...
        for (const item of m) {
          if (item.type === 'video' || (item.src && videoExtensions.includes((item.src.split('.').pop() || '').toLowerCase()))) {
            detectedVideos.push({
              ...
              type: 'video',
              src: item.src || (videoFolder + item.filename),
