...
function genBannerManifestForProject(projectName) {
  ...
  const sizeDirs = fs.readdirSync(bannersDir).filter((name) => {
    ...
    return fs.statSync(full).isDirectory() && /^\d+x\d+/.test(name);
  });

  const entries = [];
  for (const dirName of sizeDirs) {
    const sizeMatch = dirName.match(/^(\d+x\d+)/);
    if (!sizeMatch) continue;
    const size = sizeMatch[1];
    const [width, height] = size.split('x').map(n => Number(n) || 0);
    const indexPath = path.join(bannersDir, dirName, 'index.html');
